...
void displayScreen() {
  ...
  switch (currentScreen) {
    case 0: // Voltage display screen
      ...
      // Draw a rectangle around the perimeter of the screen to indicate the edit mode
      display.drawRect(0, 0, 128, 64, editMode ? WHITE : BLACK);
      break;
    case 1: // Rolling graph screen
      // Draw the graph data as a line plot
      display.drawLine(GRAPH_X, GRAPH_Y - graphData[0] / graphScale, GRAPH_X + 1, GRAPH_Y - graphData[1] / graphScale, WHITE);
      for (int i = 1; i < GRAPH_WIDTH - 1; i++) {
        display.drawLine(GRAPH_X + i, GRAPH_Y - graphData[i] / graphScale, GRAPH_X + i + 1, GRAPH_Y - graphData[i + 1] / graphScale, WHITE);
      }
      ...
      if (editMode) {
        ...
        // Highlight the selected parameter or the back arrow
        display.drawRect(currentParameter == 0 ? 64 : 0, 0, currentParameter == 0 ? 64 : 16, 16, WHITE);
      }
    ...
    case 2: // Voltage display with threshold screen
      ...
      if (editMode) {
        ...
        // Highlight the selected parameter or the back arrow
        display.drawRect(currentParameter == 0 ? 64 : 0, 24, currentParameter == 0 ? 64 : 16, 24, WHITE);
      }
...
void updateGraphData() {
  ...
  if (millis() - lastGraphUpdateTime > graphUpdateSpeed) {
    ...
    // Map the voltage value to the graph height
    graphData[GRAPH_WIDTH - 1] = map(voltage * 1000, 0, 5000, 0, GRAPH_HEIGHT);
